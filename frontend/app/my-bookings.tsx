...
export default function MyBookingsScreen() {
  ...
  const { getGuestBookings, updateBookingStatus } = useBooking();
  ...
  const loadBookings = async (page: number = pagination.page) => {
    try {
      setIsLoading(true);
      const response = await getGuestBookings({
          ...
          onPress: async () => {
            try {
              ...
              if (response.success) {
                ...
                throw new Error(response.message || 'Hủy đơn đặt phòng thất bại');
              }
            } catch (error: any) {
              console.error('Error cancelling booking:', error);
              Alert.alert('Lỗi', error.message || 'Không thể hủy đơn đặt phòng');
            } finally {
              setIsLoading(false);
            }
          },
  ...
  const filteredBookings = useMemo(() => {
    ...
    // Filter by status (already handled by API, but keep for consistency)
    if (selectedStatus) {
    ...
    // Filter by search query (homestay name, room name)
    if (searchQuery.trim()) {
  ...
  const getDaysArray = (date: Date) => {
    ...
    // Add previous month's days (Monday-first week)
    const prevMonth = new Date(date.getFullYear(), date.getMonth() - 1, 1);
    const prevMonthDays = getDaysInMonth(prevMonth);
    const daysToShow = firstDay - 1; // Days from previous month to show
    ...
    // Add next month's days to fill the grid (42 cells = 6 weeks * 7 days)
  ...
  const isCurrentMonth = (day: number | null, month: Date, dayIndex: number) => {
    ...
    // Check if this day index is within the current month range
  ...
  const handleDateSelect = (day: number | null, month: Date, dayIndex: number) => {
    ...
    // Only allow selecting days from current month
    ...
    <View style={[styles.container, { backgroundColor: isDark ? '#151718' : '#f5f5f5' }]}>
      ...
      >
        <View style={styles.headerContent}>
          ...
          <ThemedText style={styles.headerTitle}>Đơn Đặt Phòng Của Tôi</ThemedText>
      ...
      <View style={styles.searchContainer}>
        <View style={styles.searchBar}>
          ...
          <TextInput
            ...
            placeholder="Tìm kiếm theo tên homestay, phòng..."
            ...
            <ThemedText style={styles.emptySubtext}>
              {hasActiveFilters 
                ...
                : selectedStatus 
                  ...
                  : 'Tất cả đơn đặt phòng của bạn sẽ hiển thị ở đây'}
            </ThemedText>
            {hasActiveFilters && (
              <TouchableOpacity
            ...
            <View style={styles.calendarGrid}>
              {(() => {
                ...
                return days.map((day, index) => {
                  // Determine if this day is from current month
