...
import { validateEditHomestayForm } from '@/utils/homestayValidation';

export default function EditHomestayScreen() {
  ...
  useEffect(() => {
    ...
    if (user?.roleName !== 'host' && user?.roleName !== 'admin') {
      Alert.alert('Không có quyền', 'Chỉ host mới có thể chỉnh sửa homestay', [
  ...
  const loadHomestay = async () => {
    try {
      ...
      if (response.success && response.data) {
        // Convert image URLs to base64 if needed, or keep as URLs
        const homestayData = response.data;
        const imageUrls = homestayData.images || [];
        
        // Load data into form context
        loadHomestayData(homestayData);
      } else {
  ...
  const handleNext = () => {
    // Sử dụng validation từ utils
    const validation = validateEditHomestayForm(formData);
    if (!validation.isValid) {
      Alert.alert('Lỗi', validation.error);
      return;
    }

    router.push(`/add-homestay-rooms${id ? `?id=${id}` : ''}` as any);
  };
      ...
      >
        <View style={styles.headerContent}>
          ...
          <View style={styles.headerTitleContainer}>
            <ThemedText style={styles.headerTitle}>Chỉnh Sửa Homestay</ThemedText>
            <ThemedText style={styles.headerSubtitle}>Cập nhật thông tin homestay</ThemedText>
          </View>
      ...
      >
        <View style={styles.content}>
          ...
          <View style={[styles.section, styles.card]}>
            ...
            <TextInput
              style={[styles.textArea, { backgroundColor: '#fff', color: '#000' }]}
          ...
          <View style={styles.buttonContainer}>
            ...
            >
              ...
              <ThemedText style={styles.backButtonText}>Hủy</ThemedText>
            </TouchableOpacity>
